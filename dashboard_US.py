...
from queries_US import (
    get_segments, get_states, get_cities,
    _state_name_to_code,
    load_kpis, load_ytd_growth, load_otb_growth,
    load_trends, load_otb_trends,
    load_us_annual, load_us_monthly,
    ...
    HOTEL_CLASS_ORDER, EUR_TO_USD_FIXED,
)

st.set_page_config(page_title="US Hotel Performance Report", page_icon="🇺🇸", layout="wide")
...
st.title("🇺🇸 US Hotel Performance Report")
st.caption("All monetary values in native USD. Room metrics use the local currency column directly — no exchange rate conversion applied.")

# ── Sidebar ────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.header("Filters")
    st.info("📍 Hardcoded to **United States** properties only.")
    ...
    selected_state   = st.multiselect("State",   options=available_states, default=[])
    selected_city    = st.multiselect("City",    options=available_cities, default=[])
    ...
    st.caption(
        f"⚠️ Upsell values use a fixed 2024 avg rate "
        f"(1 EUR = ${EUR_TO_USD_FIXED}) as the mart has no local currency column."
    )
    if selected_state:
        st.caption("ℹ️ ~23% of US properties have no state code on file and are excluded from state-level breakdowns.")

# State names → codes for SQL (CountrySubdivisionCode format, e.g. "US-CA")
selected_state_codes = tuple(_state_name_to_code(s) for s in selected_state)

filters = {
    "segment": selected_segment,
    "state":   list(selected_state_codes),
    "city":    selected_city,
...
def render_annual_tiles(df_ann: pd.DataFrame, dim_col: str, selected_items: list,
                        global_label: str = "🇺🇸 **US Total**", item_icon: str = "📍"):
    if df_ann.empty:
        st.info("No annual data available.")
        return
    show_items = ["US Total"] + selected_items
    for metric, label, fmt in [
        ...
        for item in show_items:
            ...
            cols[0].markdown(global_label if item == "US Total" else f"{item_icon} {item}")
...
with tab2:
    ...
    if _too_few(n_props):
        ...
        with st.spinner("Loading YTD growth…"):
            ytd = load_ytd_growth(tuple(selected_segment), selected_state_codes, tuple(selected_city))
        if ytd.get("too_few"):
            st.info(TOO_FEW_MSG)
        elif ytd:
        ...
        with st.spinner("Loading OTB growth…"):
            otb_g = load_otb_growth(tuple(selected_segment), selected_state_codes, tuple(selected_city))
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 3 — Geographic Overview
# ═══════════════════════════════════════════════════════════════════════════════
with tab3:
    st.subheader("Geographic Overview")
    st.caption(
        "Full-year averages for 2024–2026 (2026 up to today), independent of date filter. "
        "Select states in the sidebar to compare against the US total. "
        "City filter narrows the universe but does not add a city-level breakdown line."
    )
    if selected_state:
        st.caption(
            "ℹ️ ~23% of US properties have no state code on record and are excluded "
            "from per-state rows (but still included in the US Total)."
        )

    with st.spinner("Loading geographic data…"):
        df_geo_ann = load_us_annual(
            tuple(selected_segment), selected_state_codes, tuple(selected_city))
        df_geo_mon = load_us_monthly(
            tuple(selected_segment), str(start_date), str(end_date),
            selected_state_codes, tuple(selected_city))

    state_color_map = {s: PALETTE[i % len(PALETTE)] for i, s in enumerate(selected_state)}
    state_color_map["US Total"] = GLOBAL_COLOR

    st.subheader("Annual Averages")
    render_annual_tiles(df_geo_ann, "state", list(selected_state),
                        global_label="🇺🇸 **US Total**", item_icon="📍")

    st.subheader("Monthly Trends")
    render_monthly_lines(df_geo_mon, "state", list(selected_state),
                         state_color_map, us_total_label="US Total")
...
with tab4:
    ...
    available_classes = [c for c in HOTEL_CLASS_ORDER if c in df_hc_ann["hotel_class"].unique()]
    ...
    render_annual_tiles(df_hc_ann, "hotel_class", selected_classes,
                        global_label="🇺🇸 **US Total**", item_icon="🏷️")

    st.subheader("Monthly Trends by Hotel Class")
    render_monthly_lines(df_hc_mon, "hotel_class", selected_classes,
                         hc_color_map, us_total_label="Global")
...
with tab6:
    ...
    st.caption(
        ...
        "Direct = Booking engine, phone, email, in-person, and all Commander origins."
    )
    ...
    with st.container(border=True):
        ...
        st.caption("Native USD — room revenue per occupied room-night, attributed to booking channel.")
        _channel_grid(df_ch_adr, "adr_usd", lambda v: f"${v:,.2f}")
        if not df_ch_adr.empty:
            excel_download_btn(df_ch_adr[["year", "channel", "room_nights", "adr_usd"]],
                               "channel_adr.xlsx")
    ...
    with st.container(border=True):
        ...
        _channel_grid(df_ch_canc, "cancel_rate", lambda v: f"{v:.1f}%", is_pp=True)

    with st.container(border=True):
        st.markdown("#### Avg Cancellation Window (days before arrival)")
        _channel_grid(df_ch_canc, "avg_cancel_window", lambda v: f"{v:.0f} days")
...
with tab7:
    ...
    if not df_pnl_monthly.empty:
        ...
        with col_pl1:
            ...
            monthly_totals = df_pnl_monthly.groupby("month")["net_revenue"].transform("sum")
            df_pnl_pct = df_pnl_monthly.copy()
            df_pnl_pct["pct"] = df_pnl_pct["net_revenue"] / monthly_totals * 100
        ...
        with col_pl2:
            ...
            fig = px.pie(df_pie, names="category", values="net_revenue",
        ...
        with st.expander("View full revenue breakdown"):
            total_all = df_pnl_mix["net_revenue"].sum()
            df_tbl = df_pnl_mix[["category", "net_revenue"]].copy()
            df_tbl["% of Total"] = (df_tbl["net_revenue"] / total_all * 100).apply(
                lambda x: f"{x:.1f}%")
            df_tbl["net_revenue"] = df_tbl["net_revenue"].apply(lambda x: f"${x:,.0f}")
    ...
    if not df_sqm.empty:
        ...
        with col_sqm1:
            ...
            fig = px.bar(df_sqm.sort_values("revenue_per_sqm"),
                         x="revenue_per_sqm", y="category", orientation="h",
                         labels={"category": "", "revenue_per_sqm": "$ per m²"},
                         color="category",
                         color_discrete_map=dict(zip(
                             ["Rooms", "Food & Beverage", "Events & Meetings",
                              "Wellness & Spa", "Sport & Recreation"], PALETTE)),
                         text=df_sqm.sort_values("revenue_per_sqm")["revenue_per_sqm"]
        ...
        with col_sqm2:
            ...
            df_sqm_tbl = df_sqm[["category", "property_count", "revenue_per_sqm"]].copy()
            df_sqm_tbl["revenue_per_sqm"] = df_sqm_tbl["revenue_per_sqm"].apply(
                lambda x: f"${x:.2f}")
    ...
    st.caption(f"⚠️ Upsell values converted from EUR using fixed 2024 avg rate (1 EUR = ${EUR_TO_USD_FIXED} USD) — no native USD column available in the mart.")
    ...
    if not df_ucat.empty:
        ...
        with col_u3:
            ...
            st.plotly_chart(fig, use_container_width=True)
        with col_u4:
            fig = px.bar(df_ucat, x="avg_value", y="category", orientation="h",
                         labels={"category": "", "avg_value": "Avg Value per Upsell ($)"},
                         color_discrete_sequence=[MAUVE],
                         text=df_ucat["avg_value"].apply(lambda x: f"${x:.2f}"))
            fig.update_traces(textposition="outside")
            fig.update_layout(yaxis={"categoryorder": "total ascending"})
            st.plotly_chart(fig, use_container_width=True)
...
with tab8:
    ...
    st.info("PowerPoint export for the US dashboard is not yet implemented. Download individual charts using the Excel buttons on each tab.")

# ── Disclaimer ─────────────────────────────────────────────────────────────────
st.divider()
geo_note = []
if selected_state:
    geo_note.append(f"State: {', '.join(selected_state)}")
if selected_city:
    geo_note.append(f"City: {', '.join(selected_city)}")
geo_str = " | ".join(geo_note) if geo_note else "All states & cities"
...
st.markdown(
    ...
    <strong>Data scope:</strong> Active Mews customers (Subscribed) in the United States as of today
    that joined at least one day before the selected start date ({str(start_date)}), and have been
    live on Mews for at least 90 days prior to the start date. Only properties with USD as their
    local currency are included in room metrics (~23% of US properties have no state code on record
    and are excluded from state-level breakdowns but included in US totals).
    Data is suppressed where fewer than {MIN_PROPERTIES} properties contribute to a metric.<br><br>
    <strong>Methodology:</strong> Occupancy, ADR and RevPAR are room-weighted market aggregates
    using native USD revenue (local currency column) — no exchange rate conversion applied.
    Upsell monetary values use a fixed 2024 annual average rate (1 EUR = ${EUR_TO_USD_FIXED} USD).
    Days with available rooms but zero occupied rooms are excluded from room metrics.<br><br>
    <strong>Filters applied:</strong>
    Country: United States (hardcoded) |
    Segment: {', '.join(selected_segment) or 'All'} |
    {geo_str}<br><br>
